...
export async function reportEmergency(lat, long) {
  try {
    ...
    // 0.04 longitude = 5 miles
    // 0.003 lat = 5 miles

    // Larger than 5 mile square

    const nearbyResponders = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.userCollectionId,
      [
        Query.lessThan("latitude", lat + 0.003),
        Query.greaterThan("latitude", lat - 0.003),
...
export const getLogs = async () => {
  try {
    ...
    return currentUserLogs.documents[0];
  } catch (error) {
    console.log(error);
  }
};
